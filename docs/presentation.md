...
| **Guardrails** | Keep autonomy safe | Gates before approval, change critic, human-in-the-loop, verification after every fix, read-only SQL without access to hidden state, tool-call caps, Jev injection flag, injection-aware prompts |
...
- Next agentic steps: `edit` decisions on the approval card, risk-tiered approvals, an `ask_human` tool, and
  budgets sized per request by Jev.
